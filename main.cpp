...
int main() {
    ...
    std::cout << "Informe o número de alunos na turma (até " << MAX_ALUNOS << "): ";
    std::cin >> numAlunos;

    if (numAlunos < 1 || numAlunos > MAX_ALUNOS) {
        std::cout << "Número de alunos inválido." << std::endl;
        return 1;
    }
    ...
    for (int i = 0; i < numAlunos; i++) {
        ...
        turma[i].Mat = i + 1;
        std::cout << "Matrícula: " << turma[i].Mat << std::endl;
        std::cout << "Nome: ";
    ...
    std::cout << "Médias dos alunos:\n";
    for (int i = 0; i < numAlunos; i++) {
        float media = calcularMedia(turma[i]);
        std::cout << "Aluno " << i + 1 << " - Matrícula: " << turma[i].Mat
                  << ", Nome: " << turma[i].Nome << ", Média: " << media << std::endl;
    }
